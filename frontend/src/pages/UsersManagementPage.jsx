...
const UsersManagementPage = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.username.trim()) {
      errors.username = t('admin.users.errors.usernameRequired') || 'Username is required';
    }
    
    if (!formData.name.trim()) {
      errors.name = t('admin.users.errors.nameRequired') || 'Name is required';
    }
    
    if (!userToEdit && !formData.password) {
      errors.password = t('admin.users.errors.passwordRequired') || 'Password is required';
    } else if (formData.password && formData.password.length < 8) {
      errors.password = t('admin.users.errors.passwordTooShort') || 'Password must be at least 8 characters';
    }
    
    if (!formData.roleId) {
      errors.roleId = t('admin.users.errors.roleRequired') || 'Role is required';
    }
    ...
    <section id="users" role="tabpanel" aria-labelledby="users-tab" className="p-4 sm:p-6 min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div className="flex justify-end mb-6">
        <button
          ...
          disabled={submitting}
        >
          <UserPlus size={18} /> <span>{t('admin.users.addUser') || 'Add User'}</span>
        </button>
      </div>
        ...
        {users.length > 0 ? (
          ...
          <div className="p-8 text-center bg-white dark:bg-gray-800">
            ...
            <p className="text-gray-500 dark:text-gray-400 mb-6">
              {t('admin.users.noUsersDescription') || 'Get started by adding your first user'}
            </p>
            <button
              onClick={() => handleOpenUserModal(null)}
              ...
              disabled={submitting}
            >
              <UserPlus size={18} /> {t('admin.users.addUser') || 'Add User'}
            </button>
          </div>
        )}
      </div>
          ...
          <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-semibold text-gray-900 dark:text-white">
                {userToEdit ? t('admin.users.editUserTitle') || 'Edit User' : t('admin.users.createUserTitle') || 'Create User'}
              </h3>
              <button
            ...
            <form onSubmit={handleSaveUser} className="space-y-4">
              <div>
                <label htmlFor="username" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                  {t('admin.users.form.username') || 'Username'} *
                </label>
                <input
                  type="text"
                  id="username"
                  ...
                  placeholder={t('admin.users.form.usernamePlaceholder') || 'Enter username'}
              ...
              <div>
                ...
                <input
                  ...
                  placeholder={t('admin.users.form.namePlaceholder') || 'Enter full name'}
                  value={formData.name}
                  onChange={handleFormChange}
                  disabled={submitting}
                />
              ...
              <div>
                <label htmlFor="password" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                  {userToEdit 
                    ? t('admin.users.form.newPassword') || 'New Password'
                    : t('admin.users.form.password') || 'Password'} 
                  {!userToEdit && ' *'}
                </label>
                <input
                  type="password"
                  ...
                  placeholder={t('admin.users.form.passwordPlaceholder') || 'Enter password'}
              ...
              <div>
                ...
                <select
                  ...
                  disabled={submitting}
                >
                  <option value="">{t('admin.users.form.selectRole') || 'Select Role'}</option>
                  {roles.map(role => (
                ...
                >
                  {submitting ? (
                    <div className="flex items-center justify-center">
                      ...
                      {t('admin.actions.processing') || 'Processing...'}
                    </div>
                  ) : userToEdit ? (
                    t('admin.actions.saveChanges') || 'Save Changes'
                  ) : (
          ...
          <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-2xl w-full max-w-md">
            <div className="text-center">
              ...
              <h3 className="mt-4 text-lg font-semibold text-gray-900 dark:text-white">
                {t('admin.users.deleteConfirm.title') || 'Confirm Deletion'}
              </h3>
              <p className="mt-2 text-gray-600 dark:text-gray-400">
                {t('admin.users.deleteConfirm.message', { name: userToDelete.name }) || `Are you sure you want to delete user "${userToDelete.name}"? This action cannot be undone.`}
              </p>
            </div>
