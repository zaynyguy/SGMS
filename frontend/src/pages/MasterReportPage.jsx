...
? escapeHtml(
JSON.stringify(activity.targetMetric)
)
: "-"
}</div>
...
? escapeHtml(
JSON.stringify(activity.previousMetric)
)
: "-"
}</div>
...
)}</th><th>${escapeHtml(t("reports.table.weight"))}</th><th>${escapeHtml(
t("reports.table.metric")
)}</th><th>${escapeHtml(t("reports.table.target"))}</th><th>${escapeHtml(
...
if (metricKey && metricKey in previousObj) previousValue = previousObj[metricKey];
else if (typeof previousObj === "number") previousValue = previousObj;
else if (previousObj && typeof previousObj === "object" && "previous" in previousObj)
