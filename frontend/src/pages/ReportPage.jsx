...
function renderMetricsList(metrics) {
  ...
  return (
    <div className="space-y-1">
      {keys.map((k) => (
        <div key={k} className="flex items-center justify-between bg-white dark:bg-gray-900 rounded px-2 py-1 border dark:border-gray-700">
        ...
        {options.map((opt) => (
          <button
            ...
            className={`flex items-center gap-2 px-3 py-2 rounded-full text-sm font-semibold transition ${
              value === opt.id ? "bg-white dark:bg-gray-700 shadow text-sky-700 dark:text-sky-300" : "text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700"
            }`}
          >
...
function fmtMonthKey(date) {
  ...
  const mm = Number(m);
  return new Date(Number(y), mm - 1, 1).toLocaleString(undefined, { month: "short", year: "numeric" });
}
        ...
        return (
          <td key={p} className="border px-3 py-3 text-sm text-gray-700 dark:text-gray-200">
            <div className="flex items-center justify-between gap-2">
              ...
              {pct !== null && !isNaN(pct) && (
                <div className="text-xs font-medium text-gray-600 dark:text-gray-300">{pct.toFixed(0)}%</div>
              )}
            </div>
          </td>
        );
...
function MasterReportPageWrapper() {
  ...
  const tableRows = useMemo(() => {
    ...
    master.goals.forEach((g) => {
      ...
      (g.tasks || []).forEach((t) => {
        rows.push({ type: "task", id: `t-${t.id}`, title: t.title, weight: t.weight ?? "-", progress: t.progress ?? "-", task: t, parentGoal: g, raw: t });
        (t.activities || []).forEach((a) => {
          rows.push({ type: "activity", id: `a-${a.id}`, title: a.title, weight: a.weight ?? "-", activity: a, parentTask: t, parentGoal: g, raw: a });
        });
      });
    });
    return rows;
  ...
  function generateHtmlForPrint() {
    ...
    const columnsHtml = periods.map((p) => {
      let label = p;
    ...
    const rowsHtml = tableRows.map((row) => {
      if (row.type === "goal") {
        return `<tr><td style="padding:8px;border:1px solid #ddd;font-weight:700">${row.title}</td><td style="padding:8px;border:1px solid #ddd">${row.weight}</td><td style="padding:8px;border:1px solid #ddd">—</td><td style="padding:8px;border:1px solid #ddd">—</td>${periods.map(() => `<td style="padding:8px;border:1px solid #ddd">—</td>`).join("")}</tr>`;
      } else if (row.type === "task") {
        return `<tr><td style="padding:8px;border:1px solid #ddd;padding-left:20px">${row.title}</td><td style="padding:8px;border:1px solid #ddd">${row.weight}</td><td style="padding:8px;border:1px solid #ddd">—</td><td style="padding:8px;border:1px solid #ddd">—</td>${periods.map(() => `<td style="padding:8px;border:1px solid #ddd">—</td>`).join("")}</tr>`;
      } else {
        const mk = pickMetricForActivity(row.activity, null);
        const targetVal = row.activity.targetMetric && mk in row.activity.targetMetric ? row.activity.targetMetric[mk] : row.activity.targetMetric?.target ?? "";
        const periodCells = periods.map((p) => {
          ...
          return `<td style="padding:8px;border:1px solid #ddd">${String(v)}</td>`;
        }).join("");
      ...
      <div style="margin-top:8px;padding-left:8px">
        ${(g.tasks || []).map((t) => `
          <div style="margin-bottom:8px">
            <div style="font-weight:600">${t.title} <span style="color:#6b7280">(${t.progress ?? 0}%)</span></div>
            ${(t.activities || []).map((a) => `
            ...
            {master.goals.map((g) => (
              <div key={g.id} className="p-4 border rounded bg-gray-50 dark:bg-gray-900">
                ...
                <div className="mt-4 pl-3 space-y-3">
                  {(g.tasks || []).map((t) => (
                    <div key={t.id}>
                      <div className="text-lg font-semibold text-gray-800 dark:text-gray-100">{t.title} <span className="text-sm text-gray-400">({t.progress ?? 0}%)</span></div>
