...
export default function SubmitReportInline({ data, onClose, onSubmit, loading, t }) {
  ...
  useEffect(() => {
    ...
    const rawTargets = safeParseJson(data.targetMetric ?? data.activity?.targetMetric ?? data.target_metrics ?? {});
    const rawCurrents = safeParseJson(data.currentMetric ?? data.activity?.currentMetric ?? data.current_metrics ?? {});

    const targets = rawTargets && typeof rawTargets === "object" ? rawTargets : {};
    const currents = rawCurrents && typeof rawCurrents === "object" ? rawCurrents : {};
  ...
  }, [data?.activityId, data?.targetMetric, data?.currentMetric, data?.target_metrics, data?.current_metrics, data]);
  ...
  const { totalTarget, totalCurrent, totalAvailable, isNumericTarget } = (() => {
    const rawTargets = safeParseJson(data?.targetMetric ?? data?.activity?.targetMetric ?? {});
    const rawCurrents = safeParseJson(data?.currentMetric ?? data?.activity?.currentMetric ?? {});
    const tObj = rawTargets && typeof rawTargets === "object" ? rawTargets : {};
    const cObj = rawCurrents && typeof rawCurrents === "object" ? rawCurrents : {};

    const totalT = Object.values(tObj).reduce((s, v) => s + (extractNumeric(v) || 0), 0);
    const totalC = Object.values(cObj).reduce((s, v) => s + (extractNumeric(v) || 0), 0);
    return {
      totalTarget: totalT,
      totalCurrent: totalC,
  ...
  const handleSubmit = async (e) => {
    ...
    const hasNaN = metricsArray.some((m) => m && String(m.value).trim() !== "" && extractNumeric(m.value) === null);
    if (hasNaN) {
    ...
    await onSubmit({
      ...
      metrics_data: Object.keys(metricsObj).length ? JSON.stringify(metricsObj) : null,
      ...
      >
        ...
        >
          ...
          <div className="px-6 py-5 space-y-4 overflow-y-auto">
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">{t("project.labels.activity")}</label>
            ...
            <div>
              <label htmlFor="narrative" className="block text-sm font-medium text-gray-700 dark:text-gray-300">{t("project.fields.narrative")}</label>
              <textarea
                id="narrative"
                value={narrative}
            ...
            {isNumericTarget && (
              <div className="p-3 rounded-lg bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-800 text-indigo-800 dark:text-indigo-300 text-sm">
                {t("project.report.overallHint", { target: totalTarget, used: totalCurrent, remaining: totalAvailable }) || `Overall Target: ${totalTarget}. Total progress: ${totalCurrent}. Remaining: ${totalAvailable}.`}
              </div>
            )}

            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">{t("project.labels.metrics")}</label>
              <div className="mt-2 space-y-3">
                {metricsArray.map((m, idx) => {
                  const remaining = m.target !== null ? (m.target - (m.current || 0)) : null;
                  return (
                    <div key={m.id} className="p-3 border rounded-lg space-y-2">
                      <div className="flex gap-2 items-center">
                        <input
                          ...
                          className={`flex-1 px-2 py-1 border rounded-lg text-sm ${m.isPredefined ? 'bg-gray-100 dark:bg-gray-900' : 'bg-white dark:bg-gray-700'}`}
                        />
                        <input
                          type="number"
                          step="any"
                          ...
                          className="flex-1 px-2 py-1 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-sm"
                        />
                        <button
                          type="button"
                      ...
                      {m.isPredefined && m.target !== null && (
                        <div className="text-xs text-gray-500 dark:text-gray-400 pl-1">
                          {t("project.hints.metricRemaining", { current: m.current, target: m.target, remaining }) || `Current: ${m.current} / ${m.target} (Remaining: ${remaining})`}
                        </div>
                      )}
                    </div>
                  );
              ...
              <button type="button" onClick={addMetricRow} className="mt-3 px-3 py-1.5 bg-green-600 text-white rounded-lg text-xs font-medium">
                <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg>
                {t("project.actions.addMetric") || "Add Metric"}
              </button>

              {localErr && (
                <div className="text-xs text-red-500 mt-2 p-2 bg-red-50 dark:bg-red-900/20 rounded-lg border border-red-200 dark:border-red-800" style={{ animation: "shake 0.4s ease-in-out" }}>
                  {localErr}
                </div>
              )}
            </div>
            ...
            <div>
              ...
              <select id="newStatus" value={newStatus} onChange={(e) => setNewStatus(e.target.value)} className="mt-1 w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">{t("project.labels.attachments")}</label>
              <label htmlFor="file-upload" className="mt-2 flex justify-center items-center gap-2 px-6 py-4 border-2 border-gray-300 dark:border-gray-600 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:bg-gray-700/50">
                <UploadCloud className="h-6 w-6 text-gray-500 dark:text-gray-400" />
                <span className="text-sm text-gray-600 dark:text-gray-300">{t("project.actions.uploadFiles") || "Click to upload files (max 5)"}</span>
              </label>
              ...
              {files.length > 0 && (
                <div className="mt-3 space-y-1">
                  ...
                  <ul className="text-xs text-gray-700 dark:text-gray-200 space-y-1">
                    {files.map((f, index) => (
                      <li key={f.id} className="flex items-center gap-2 p-2 bg-gray-100 dark:bg-gray-700 rounded-lg">
                        <Paperclip className="h-3 w-3 flex-shrink-0" />
                        <span className="flex-1 truncate">{f.file.name}</span>
                        <span className="text-gray-500 dark:text-gray-400">({Math.round(f.file.size / 1024)} KB)</span>
                        <button type="button" onClick={() => removeFile(f.id)} className="ml-2 p-1 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/20 text-gray-600">
                          <X className="h-4 w-4" />
                        </button>
                      </li>
                    ))}
          ...
          <div className="px-6 py-4 border-t border-gray-200 dark:border-gray-700 flex justify-end gap-3 bg-gray-50 dark:bg-gray-800">
            <button type="button" onClick={onClose} className="px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-sm font-medium">
              {t("project.actions.cancel") || "Cancel"}
            </button>

            <button type="submit" disabled={loading} className="px-4 py-2 rounded-lg bg-indigo-600 text-white text-sm font-medium disabled:opacity-50 flex items-center">
