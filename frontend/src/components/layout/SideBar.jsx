...
const Sidebar = ({
  ...
  const mainMenuItems = [
    ...
    hasPermission("manage_access") && {
      ...
      children: [
        {
          to: "/accessmanagement/usermanagement",
          label: t("sidebar.menu.userManagement") || "User Management",
          icon: <User size={16} />,
        },
        {
          to: "/accessmanagement/rolemanagement",
          label: t("sidebar.menu.roleManagement") || "Role Management",
          icon: <Shield size={16} />,
        },
        {
          to: "/accessmanagement/groupmanagement",
          label: t("sidebar.menu.groupManagement") || "Group Management",
          icon: <Layers size={16} />,
        },
      ],
    },
    ...
    hasPermission("manage_reports") && {
      ...
      children: [
        {
          to: "/report/review",
          label: t("sidebar.menu.reportReview") || "Report Review",
          icon: <ClipboardCheck size={16} />,
        },
        {
          to: "/report/master",
          label: t("sidebar.menu.masterReport") || "Master Report",
          icon: <Table2 size={16} />,
        },
      ],
    },
    ...
    .map((it) => {
      ...
      return {
        ...
        children: it.children.filter((c) =>
          c.permission ? hasPermission(c.permission) : true,
        ),
      };
    });

  useEffect(() => {
    const found = mainMenuItems.find(
      (it) =>
        it.children &&
        it.children.some((ch) => location.pathname.startsWith(ch.to)),
    );
    setOpenMenu(found ? found.key : null);
  }, [location.pathname]);

  const effectiveMobileOpen =
    typeof mobileIsOpen === "boolean" ? mobileIsOpen : ctxOpen;
  ...
  const MenuItem = ({ item, showExpanded }) => {
    ...
    return hasChildren ? (
      <div>
        ...
        >
          <div
            className={`mt-1 flex flex-col gap-1 px-1 ${showExpanded ? "ml-6" : "ml-0"}`}
          >
          ...
          return (
            ...
            >
              <div style={{ zIndex: 1 }} className="flex items-center">
                <div
                  className={`flex-shrink-0 flex items-center justify-center w-5 h-5 rounded-lg transition-all duration-200`}
                >
                  {React.cloneElement(item.icon, { size: 20 })}
                </div>
  ...
  return (
    <div
      className="relative min-h-screen bg-[var(--background)] text-[var(--on-background)] dark:bg-gray-900 dark:text-gray-100"
      style={{
    ...
    >
        ...
        <div className="flex flex-col h-full overflow-hidden">
          <div className="flex items-center justify-center p-3 h-16 border-b border-[var(--outline)]/[0.12] dark:border-gray-700 bg-[var(--background)] dark:bg-gray-800">
            <div className="flex items-center min-w-0 transition-all duration-300">
              <div
                className={`${showExpanded ? "mr-2" : "mx-auto"} flex-shrink-0`}
              >
                <img
                  src={companyLogo}
          ...
          <div className="mt-auto p-3 bg-[var(--background)] dark:bg-gray-800 border-t border-[var(--outline)]/[0.12] dark:border-gray-700">
            <div
              className={`flex items-center gap-3 ${showExpanded ? "justify-start" : "justify-center"}`}
            >
              <div
                className={`flex items-center ${showExpanded ? "min-w-0" : "justify-center"}`}
              >
                ...
                {showExpanded && (
                  <div className="min-w-0 ms-2">
                    <div className="font-medium text-[var(--on-surface)] dark:text-white truncate text-sm leading-tight">
                      {formatName(user?.name)}
                    </div>
                    <div className="text-xs text-[var(--on-surface-variant)] dark:text-gray-400 truncate">
                      {user?.role || ""}
                    </div>
                  </div>
                )}
      ...
      <div
        className={`transition-all duration-300 ${contentMargin} min-h-screen bg-white dark:bg-gray-900`}
      >
        {children}
      </div>
    </div>
