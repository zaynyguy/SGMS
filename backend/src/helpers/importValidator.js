...
function buildDedupKey(rows, type) {
  switch (type) {
    ...
    case "task": {
      const goalKey = rows.goal_id
        ? `goalId:${String(rows.goal_id).trim()}`
        : rows.goal_roll_no
          ? `goalRoll:${String(rows.goal_roll_no).trim()}`
          : `goalTitle:${normalize(rows.goal_title || rows.goal_name)}`;
      if (rows.task_roll_no)
        return `${goalKey}|rollno:${String(rows.task_roll_no).trim()}`;
      return `${goalKey}|title:${normalize(rows.task_title || rows.task_name || rows.title || rows.name)}`;
    }
    case "activity": {
      const taskKey = rows.task_id
        ? `taskId:${String(rows.task_id).trim()}`
        : rows.task_roll_no
          ? `taskRoll:${String(rows.task_roll_no).trim()}`
          : `taskTitle:${normalize(rows.task_title || rows.task_name)}`;
      if (rows.activity_roll_no)
        return `${taskKey}|rollno:${String(rows.activity_roll_no).trim()}`;
...
function dedupeRows(rows, type, warnings) {
  ...
  for (const row of rows) {
    ...
    if (seen.has(key)) {
      warnings.push({
        sheet:
          type === "goal" ? "Goals" : type === "task" ? "Tasks" : "Activities",
        row: row.rowNumber || null,
        message: `Duplicate ${type} entry removed based on roll number/title match.`,
      });
      continue;
    }
    seen.set(key, row);
...
function validateImportData(parsed) {
  ...
  if (!parsed.goals.length) {
    errors.push({
      sheet: "Goals",
      message: "Goals sheet is empty or missing.",
    });
  }
  if (!parsed.tasks.length) {
    errors.push({
      sheet: "Tasks",
      message: "Tasks sheet is empty or missing.",
    });
  }
  if (!parsed.activities.length) {
    errors.push({
      sheet: "Activities",
      message: "Activities sheet is empty or missing.",
    });
  }
  ...
  parsed.goals.forEach((row, index) => {
    ...
    const title = buildIdentifier(row, [
      "goal_title",
      "goal_name",
      "title",
      "name",
    ...
    if (!title && !code) {
      errors.push({
        sheet: "Goals",
        row: rowNumber,
        message: "Goal must include a title or code.",
      });
    }
    ...
    if (identifier) {
      ...
      if (knownGoalIdentifiers.has(key)) {
        warnings.push({
          sheet: "Goals",
          row: rowNumber,
          message: "Duplicate Goal entry detected.",
        });
      } else {
  ...
  parsed.tasks.forEach((row, index) => {
    ...
    const goalIdentifier = buildIdentifier(row, [
      "goal_code",
      "goal_id",
      "goal_roll_no",
      "goal_title",
    ...
    if (!title) {
      errors.push({
        sheet: "Tasks",
        row: rowNumber,
        message: "Task must include a title.",
      });
    }
    if (!goalIdentifier) {
      errors.push({
        sheet: "Tasks",
        row: rowNumber,
        message: "Task must reference a Goal by title, id, or code.",
      });
    }
  ...
  parsed.activities.forEach((row, index) => {
    ...
    const taskIdentifier = buildIdentifier(row, [
      "task_code",
      "task_id",
      "task_title",
      "task_name",
    ...
    if (!title) {
      errors.push({
        sheet: "Activities",
        row: rowNumber,
        message: "Activity must include a title.",
      });
    }
    if (!taskIdentifier) {
      errors.push({
        ...
        row: rowNumber,
        message:
          "Activity must reference a Task by title, id, code, or roll number.",
      });
    }
    ...
    for (let q = 1; q <= 4; q += 1) {
      ...
      const plannedValue = row[`q${q}_planned`];
      const actualValue = row[`q${q}_actual`];
      const remarkValue = row[`q${q}_remark`] ?? row[`q${q}_remarks`];
      ...
      if (
        ...
        if (!Number.isFinite(parsedGoal)) {
          errors.push({
            sheet: "Activities",
            row: rowNumber,
            message: `Q${q} Goal must be a number or empty.`,
          });
        }
      }
      if (
        ...
        if (!Number.isFinite(parsedRecord)) {
          errors.push({
            sheet: "Activities",
            row: rowNumber,
            message: `Q${q} Record must be a number or empty.`,
          });
        }
      }
      if (
        ...
        plannedValue !== null &&
        String(plannedValue).trim() !== ""
      ) {
        const parsedPlanned = Number(plannedValue);
        if (!Number.isFinite(parsedPlanned)) {
          errors.push({
            sheet: "Activities",
            row: rowNumber,
            message: `Q${q} Planned must be a number or empty.`,
          });
        }
      }
      if (
        ...
        if (!Number.isFinite(parsedActual)) {
          errors.push({
            sheet: "Activities",
            row: rowNumber,
            message: `Q${q} Actual must be a number or empty.`,
          });
        }
      }
      if (
        ...
        errors.push({
          sheet: "Activities",
          row: rowNumber,
          message: `Q${q} Remark must be text or empty.`,
        });
      }
    }
    ...
    if (knownActivityIdentifiers.has(identifier)) {
      warnings.push({
        sheet: "Activities",
        row: rowNumber,
        message: "Duplicate Activity entry detected.",
      });
    } else {
  ...
  parsed.quarters.forEach((row, index) => {
    const rowNumber = row.rowNumber || index + 2;
    const activityIdentifier = buildIdentifier(row, [
      "activity_code",
      "activity_id",
      "activity_roll_no",
      "activity_title",
    ...
    if (!activityIdentifier) {
      errors.push({
        sheet: row.sheetName || `Quarter${row.quarter}`,
        row: rowNumber,
        message:
          "Quarter update must reference an Activity by id, roll number, title, or code.",
      });
    }
    if (row.quarter < 1 || row.quarter > 4) {
      warnings.push({
        sheet: row.sheetName || `Quarter${row.quarter}`,
        row: rowNumber,
        message: "Quarter number should be between 1 and 4.",
      });
    }
  });
