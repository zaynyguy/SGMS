...
// getAllUsers, createUser, deleteUser functions remain the same...
...
exports.createUser = async (req, res) => {
  ...
  try {
    ...
    const query = `
      ...
      VALUES ($1, $2, $3, $4) RETURNING id, username, name, "roleId";`;
    ...
    if (error.code === '23505') return res.status(409).json({ message: 'A user with that username already exists.' });
    res.status(500).json({ message: 'Internal server error' });
  }
};
...
exports.updateUser = async (req, res) => {
  const { id } = req.params;
  const { username, name, roleId, password } = req.body; // Password is now an optional field
  ...
  try {
    ...
    if (password && password.trim() !== '') {
      ...
      `;
      queryParams = [username, name, roleId, hashedPassword, id];
    } else {
      ...
      query = `
        ...
        RETURNING id, username, name, "roleId";
      `;
      queryParams = [username, name, roleId, id];
    }
