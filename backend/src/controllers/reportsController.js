...
exports.submitReport = async (req, res) => {
  ...
  try {
    ...
    // notify managers and assignee (best-effort)
    try {
...
`,
    ...
    if (Object.keys(historyByActivity).length === 0) {
      ...
      for (const [actId, reports] of Object.entries(historyByActivity)) {
        reportBasedBreakdowns[actId] = { monthly: {}, quarterly: {}, annual: {} };
        for (const rep of reports) {
          ...
          if (!reportBasedBreakdowns[actId].monthly[monthKey]) reportBasedBreakdowns[actId].monthly[monthKey] = [];
          reportBasedBreakdowns[actId].monthly[monthKey].push(rep);
          if (!reportBasedBreakdowns[actId].quarterly[quarterKey]) reportBasedBreakdowns[actId].quarterly[quarterKey] = [];
          reportBasedBreakdowns[actId].quarterly[quarterKey].push(rep);
          if (!reportBasedBreakdowns[actId].annual[yearKey]) reportBasedBreakdowns[actId].annual[yearKey] = [];
      ...
      for (const goal of masterJson.goals || []) {
        for (const task of goal.tasks || []) {
          for (const activity of task.activities || []) {
            activity.history = reportBasedBreakdowns[activity.id] || { monthly: {}, quarterly: {}, annual: {} };
          }
        }
      }
    ...
    for (const [actId, snaps] of Object.entries(historyByActivity)) {
      ...
      for (const snap of snaps) {
        ...
        breakdowns[numericActId].monthly[monthKey].push({ date: dateStr, progress: snap.progress, metrics: snap.metrics, recorded_at: snap.recorded_at });
        if (!breakdowns[numericActId].quarterly[quarterKey]) breakdowns[numericActId].quarterly[quarterKey] = [];
        breakdowns[numericActId].quarterly[quarterKey].push({ date: dateStr, progress: snap.progress, metrics: snap.metrics, recorded_at: snap.recorded_at });
        if (!breakdowns[numericActId].annual[yearKey]) breakdowns[numericActId].annual[yearKey] = [];
        breakdowns[numericActId].annual[yearKey].push({ date: dateStr, progress: snap.progress, metrics: snap.metrics, recorded_at: snap.recorded_at });
      }
    }
