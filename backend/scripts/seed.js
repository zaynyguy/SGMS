...
async function seedDatabase() {
  ...
  try {
    ...
    for (const roleName of ["Admin", "Manager", "User"]) {
      if (!roles[roleName]) {
        const res = await db.query(`SELECT id FROM "Roles" WHERE name = $1`, [
          roleName,
        ]);
  ...
  `);
    ...
    `);
    const devGroupId =
      devGroupRes.rows[0]?.id ||
      (await db.query(`SELECT id FROM "Groups" WHERE name='Development Team'`))
        .rows[0].id;
    ...
    const marketingGroupRes = await db.query(`
      ...
      RETURNING id;
    `);
    const marketingGroupId =
      marketingGroupRes.rows[0]?.id ||
      (await db.query(`SELECT id FROM "Groups" WHERE name='Marketing Team'`))
        .rows[0].id;
    ...
    const managerId =
      ...
      (await db.query(`SELECT id FROM "Users" WHERE username='manager'`))
        .rows[0].id;
    ...
    const dev1Id =
      dev1Res.rows[0]?.id ||
      (await db.query(`SELECT id FROM "Users" WHERE username='dev1'`)).rows[0]
        .id;
    ...
    const marketerId =
      marketerRes.rows[0]?.id ||
      (await db.query(`SELECT id FROM "Users" WHERE username='marketer1'`))
        .rows[0].id;
